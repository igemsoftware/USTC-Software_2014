package KShortest{
	...
	public class LinkWayPreviewer extends Sprite{
		...
		{
			if(GxmlContainer.Block_space[nodes[0].ID]!=null&&GxmlContainer.Block_space[nodes[nodes.length-1].ID]!=null){
				
				var sx:Number=GxmlContainer.Block_space[nodes[0].ID].Position[0];
				var sy:Number=GxmlContainer.Block_space[nodes[0].ID].Position[1];
				
				var ex:Number=GxmlContainer.Block_space[nodes[nodes.length-1].ID].Position[0];
				var ey:Number=GxmlContainer.Block_space[nodes[nodes.length-1].ID].Position[1];
		...
		public function GiveNodes(arr:Array):void{
			...
			for (var i:int = 0; i < arr.length; i++) {
				nodes.push(new BlastResult(arr[i].NAME,arr[i].TYPE,arr[i]._id));
			}
			dg.visible=true;
