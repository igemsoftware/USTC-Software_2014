...
public static function loadCompressedBlock(id:String,_name:String,px:Number,py:Number,biotp:String,detail:String=null):CompressedNode {
	if (Block_space[id]==null) {
...
public static function loadBlock(id:String,t:String,px:Number,py:Number,biotp:String,detail:String=null):CompressedNode {
	if (Block_space[id]==null) {
		var tar:CompressedNode=Block_space[id]=new CompressedNode(id,t,NodeTypeInit.BiotypeList[biotp],px,py,detail);
		
		tar.TextMap=TextMapLoader.generateTextMap(t);
		tar.textY=Math.round(tar.skindata.radius/2);	
...
private static function setpos(e=null):void{
	...
	if(CurrentBlock.NodeLink.ID.length==24){
		GxmlContainer.PreExpand(CurrentBlock.NodeLink);
	}
